...
class PageLayout {
    ...
    global $debugger; if (isset($debugger)) $debugger->et('До контента');
		...
		//}
        global $debugger; if (isset($debugger)) $debugger->et('До скина');
		// Скин
	...
	function getGlobalMenu() {
		...
		} else if (($msc->db != '' && $msc->table == '') || $msc->page == 'tbl_list') {
			$type = 'db';
			$dbMenu = array_merge(array(
      ...
      if (!empty($array[2])) {
        ...
        $curl = $url;
			...
			} else if (stristr($action, 'truncate')) {
	...
	function getTableMenu($selector = false, $auto = true) {
		...
		$selectorTables = null;
		$msc->selectDb($msc->db);
		global $debugger; if (isset($debugger)) $debugger->et('До getCashedTablesArray');
